...
class FrontEnd:
    ...
    def frames_to_magspec(self, frames):
        ...
        # compute magnitude
        # ??
        magspec = np.zeros([self.fft_size // 2 + 1, frames.shape[1]])
        for i in range(frames.shape[1]):
            frame_fft = np.fft.fft(frames[:, i])[:self.fft_size // 2 + 1]
            frame_mag = np.abs(frame_fft) ** 2
            magspec[:, i] = frame_mag
        return magspec
    ...
    def magspec_to_fbank(self, magspec):
        ...
        #print(magspec.shape, self.mel_filterbank.shape)
    ...
    def mean_norm_fbank(self, fbank):
        ...
        mean_fbank = np.mean(fbank, axis=1, keepdims=True)
        # subtract it from each frame
        fbank = fbank - mean_fbank
        return fbank
